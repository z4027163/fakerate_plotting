...
TH1D* playfit(TString which_file, TString path, TString option, TString opt_point, TString binning) {
  ...
  for(int bin=0;bin<n_bin;++bin){
    ...
    if(opt_point.Contains("bdt47")){
      name = Form("pt_muid47_bin%d", bin);
      pt_yield_muid = do_fit(paramval,option,Form("hv0_Mass_muid_47_bin%d",bin),"hv0_Mass_muid_47_allbin", which_file, name, path, opt_point, binning);
    }              
    else if(opt_point.Contains("bdt63")){
      name = Form("pt_muid63_bin%d", bin);
      pt_yield_muid = do_fit(paramval,option,Form("hv0_Mass_muid_63_bin%d",bin),"hv0_Mass_muid_63_allbin", which_file, name, path, opt_point, binning);
    }else if(opt_point.Contains("bdt74")){
      name = Form("pt_muid74_bin%d", bin);
      pt_yield_muid = do_fit(paramval,option,Form("hv0_Mass_muid_74_bin%d",bin),"hv0_Mass_muid_74_allbin", which_file, name, path, opt_point, binning);
    }else if(opt_point.Contains("bdt83")){
      name = Form("pt_muid83_bin%d", bin);
      pt_yield_muid = do_fit(paramval,option,Form("hv0_Mass_muid_83_bin%d",bin),"hv0_Mass_muid_83_allbin", which_file, name, path, opt_point, binning);
    }
  ...
  // cout<<"bin7 "<<hv0_fakeid_pt->GetBinContent(7)<<"\t err "<<hv0_fakeid_pt->GetBinError(7)<<endl;;
   ...
   // else
   hv0_fakeid_pt->SetMaximum(0.009);
   ...
   if(which_file.Contains("2022"))title3 = "2022";
   if(which_file.Contains("2023"))title3 = "2023";
   TString title4 = "muon mva = bmm6";
   if(opt_point.Contains("bdt47"))title4="muon mva>0.47";
   if(opt_point.Contains("bdt63"))title4="muon mva>0.63";
   if(opt_point.Contains("bdt74"))title4="muon mva>0.74";
   if(opt_point.Contains("bdt83"))title4="muon mva>0.83";
   if(opt_point.Contains("softid"))title4="muon mva = looseid";
   if(opt_point.Contains("mediumid"))title4="muon mva = mediumid";
   
   if(string::npos != tmpstring.find("Data") && (string::npos != tmpstring.find("2022")))filenamechange="Data_529_2022_ks";
   if(string::npos != tmpstring.find("MC") && (string::npos != tmpstring.find("2022")))filenamechange="MC_529_2022_ks";

   if(string::npos != tmpstring.find("Data") && (string::npos != tmpstring.find("2023")))filenamechange="Data_529_2023_ks";
   if(string::npos != tmpstring.find("MC") && (string::npos != tmpstring.find("2023")))filenamechange="MC_529_2023_ks";
...
void fitks_loop(TString syear, TString binning){
   ...
   ////Final set of files
   TString path = "ks/";
   TString Data_Egamma_file = Form("data_EGamma_529_2022_ks_trigger_%s", binning.Data());
   TString Data_Parking_file = Form("data_parking_529_2022_ks_%s", binning.Data());
   TString MC_TT_file= Form("MC_TT_529_2022_ks_%s", binning.Data());
   TString MC_W_file= Form("MC_W_529_2022_ks_%s", binning.Data());
   TString MC_DY_file= Form("MC_DY_529_2022_ks_%s", binning.Data());
   TString MC_combined_file= Form("MC_combined_529_2022_ks_%s", binning.Data());
   TString Data_combined_file= Form("data_combined_529_2022_ks_trigger_%s", binning.Data());
   TString Data_Egamma_file_2023 = Form("data_EGamma_529_2023_ks_trigger_%s", binning.Data());
